...
한우세무법인 동대문점의 웹사이트 및 관리자 시스템을 구축하기 위한 프로젝트입니다.
...
- 복잡한 MSA 인프라를 한번에 해결 가능(Gateway, Load Balancer, Service Discovery, Service Mash 등)
...
1. 파일 업로드 시 관리 문제
    ...
    해결 : Persistant Volume Claim을 사용하여 리소스 서버와 프론트엔드 웹 서버들이 같은 저장소를 바라보게 하여 스트림를 쓰지 않고 직접 저장소를 통해서 가져오게 함. 네트워크를 사용하지 않아서 훨씬 안정적이고 빠름.
