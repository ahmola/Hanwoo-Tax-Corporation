...
이러한 문제를 해결 및 추가 요구사항으로,

- 상담 문의 관리 시스템(이메일 알람 기능)
...
- 낮은 메모리 점유율과 경량 컨테이너 이미지로 컴퓨팅 리소스 소비 최소화.
  ...
  예시로, 똑같은 기능이여도 Spring으로 만들면 500MB이상의 메모리 리소스를 차지하지만, Go로 만들면 10MB(Spring의 1/50)도 안되는 리소스에 더욱 빠른 성능(ms -> μs로 단위가 바뀜)을 보여줌.
